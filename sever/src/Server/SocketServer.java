...
public class SocketServer implements Runnable {
            ...
            if(msg.type.equals("login")){
                if(findUserThread(msg.sender) == null){
                    if(db.checkLogin(msg.sender, msg.content)){
                        ...
                        //checking username first or not
                        if(userNameList != null){
                             for(int i = 0; i < userNameList.size(); i++){
                                if(userNameList.get(i).equals(msg.sender)){
                                    first_time = false;
                                }
                        }
                        }else{
                            userNameList.add(msg.sender);
                        }
                ...
                if(msg.recipient.equals("All Users")){
                ...
                if(msg.recipient.equals("All Users")){
                    clients[findClient(ID)].send(new Message("message", "SERVER", "Uploading to 'All users' forbidden", msg.sender));
                }
                else{
    ...
    public void Announce(String type, String sender, String content){
        Message msg = new Message(type, sender, content, "All Users");
        for(int i = 0; i < clientCount; i++){
            clients[i].send(msg);
        }
    }
